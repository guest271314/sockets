    ...
    write(c) {
      if (this.muted) {
        ...
        if (c.match(/^\u001b/)) {
          ...
          if (
            this._prompt && this._hadControl && c.indexOf(this._prompt) === 0
          ) {
            this._hadControl = false;
...
var require_read = __commonJS((exports, module) => {
  ...
  module.exports = async function read({
    ...
    if (
      typeof def !== "undefined" && typeof def !== "string" &&
      typeof def !== "number"
    ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = timeout &&
        setTimeout(() => onError(new Error("timed out")), timeout);
      output.unmute();
      rl.setPrompt(prompt);
      rl.prompt();
      if (silent) {
...
function isBuffer(value) {
  return value && value.constructor && value.constructor.isBuffer &&
...
var useBuffer = globalThis.process && !globalThis.process.browser &&
  globalThis.Buffer && typeof globalThis.Buffer.isBuffer === "function";
...
function readUint32(data, offset, options) {
  ...
  const value = data[offset] * 16777216 + (data[offset + 1] << 16) +
...
function readUint64(data, offset, options) {
  ...
  const hi = data[offset] * 16777216 + (data[offset + 1] << 16) +
    (data[offset + 2] << 8) + data[offset + 3];
  const lo = data[offset + 4] * 16777216 + (data[offset + 5] << 16) +
...
function encodeNegint(buf, token3) {
  ...
  const unsigned = typeof negint === "bigint"
    ? negint * neg1b - pos1b
    : negint * -1 - 1;
...
function tokenBytes(token4) {
  if (token4.encodedBytes === undefined) {
    token4.encodedBytes = token4.type === Type.string
      ? fromString(token4.value)
      : token4.value;
  }
  ...
  if (inp === Infinity) {
    ...
    if (exponent === 255) {
      ...
      if (logicalExponent < -24) {
        ...
        dataView.setUint16(
          0,
          (valu32 & 2147483648) >> 16 | logicalExponent + 15 << 10 |
            mantissa >> 13,
          false,
        );
      }
...
encodeFloat.encodedSize = function encodedSize8(token9, options) {
  const float = token9.value;
  if (
    float === false || float === true || float === null || float === undefined
  ) {
    return 1;
  }
  if (!options || options.float64 !== true) {
...
for (
  const typ
    of "Uint8Clamped Uint16 Uint32 Int8 Int16 Int32 BigUint64 BigInt64 Float32 Float64"
      .split(" ")
) {
...
function tokensToLength(
  tokens,
  encoders = cborEncoders2,
  options = defaultEncodeOptions2,
) {
  if (Array.isArray(tokens)) {
    let len = 0;
...
class IntegrityBlockSigner {
  ...
  async sign() {
    ...
    for (const signingStrategy of this.signingStrategies) {
      ...
      const newAttributes = {
        // diff
        [getPublicKeyAttributeName(publicKey)]: await getRawPublicKey(
          publicKey,
        ),
      };
...
class WebBundleId {
  ...
  async serialize() {
    return base32Encode(
      new Uint8Array([...await getRawPublicKey(this.key), ...this.typeSuffix]),
      "RFC4648",
      { padding: false },
    ).toLowerCase();
  }
...
export { bundleIsolatedWebApp, WebBundleId };
