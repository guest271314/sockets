...
(async () => {
  const listener = await tjs.listen("udp", "127.0.0.1", "8000");
  const { family, ip, port } = listener.localAddress;
  sendMessage(encodeMessage(`Listening on family: ${family}, ip: ${ip}, port: ${port}`));
  try {
    const buffer = new Uint8Array(65536);
    let rinfo;
    while (true) {
      rinfo = await listener.recv(buffer);
      const data = buffer.subarray(0, rinfo.nread);
      await listener.send(data, rinfo.addr);
    }
  } catch (e) {
    sendMesage(encodeMessage(e.message));
    listener.close();
